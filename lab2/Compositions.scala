object Compositions {
  ...
  def main(args: Array[String]): Unit ={
    val number: Int = 97
    println("First: " + Compositions.testCompose((number: Int) => number.toLong)((number: Long) => number.toChar)((symbol: Char) => symbol.toString)(number))
    println("Second: " + Compositions.testMapFlatMap((number:Int) => Some(number))((number: Int) => Some(number * number))((number: Int) => number * number * number)(Some(2)))
    println("Third: " + Compositions.testForComprehension((number:Int) => Some(number + number))((number: Int) => Some(number + number))((number:Int) => number + number)(Some(2)))
  }
}